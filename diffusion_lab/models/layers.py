...
Based on Appendix B from "DDPM" paper
 ...
 - all models have 32x32 resolution between the convolutional blocks
...
class ConvBlock(nn.Module):
	def __init__(self, in_channels, out_channels, groups=8):
		...
		self.norm = nn.Identity() if out_channels % groups != 0 else nn.GroupNorm(groups, out_channels)
		self.act = nn.SiLU()
...
class SelfAttentionBlock(nn.Module):
	...
	def __init__(self, num_heads, in_channels, num_groups=32, embedding_dim=256):
		...
		# For each of the heads use d_k = d_v = d_model / num_heads
	...
	def split_features_for_heads(self, tensor):
		...
		# K, Q and V respectively, according to the paper.
	...
	def forward(self, input_tensor):
		...
		# Perform linear projection and reshape the tensor such that
